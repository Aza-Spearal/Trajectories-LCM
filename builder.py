...
class BaseLCModel(AbstractLCModel):
    ...
    def __init__(
        ...
        if layernorm:
            self.prelayernorm = torch.nn.LayerNorm(self.sonar_embed_dim).to('cuda')
            self.postlayernorm = torch.nn.LayerNorm(self.sonar_embed_dim).to('cuda')
        
        import numpy as np
        self.pad = torch.from_numpy(np.load('/workspace/eloise/sentemb/data/pad.npy'))
    ...
    #) -> EmbeddingsBatch:
        ...
        seq_len = count_before_first_pad(batch_tens, self.pad.to(batch_tens.device))
        
        assert (
            len(seq_len) == batch_tens.size(0)
        ), "Batch size ("+str(batch_tens.size(0))+") doesn't correspond to the batch padder ("+str(len(seq_len))+")"
        
        batch = EmbeddingsBatch(batch_tens)
        ...
        elif hasattr(self, 'prelayernorm'):
            batch = EmbeddingsBatch(self.prelayernorm(batch.seqs))
        
        #batch.size(1) should correspond to self.config.max_seq_len
        batch.padding_mask = PaddingMask(seq_len, batch_tens.size(1))
        ...
        elif hasattr(self, 'postlayernorm'):
            batch = EmbeddingsBatch(self.postlayernorm(batch.seqs))

        #return predicted_means
        return predicted_means.seqs
...
class BaseLCModelBuilder(AbstractLCModelBuilder):
    ...
    def build_model(self, layernorm) -> BaseLCModel:
        """Build a model."""
